<template>
  <aside class="space-y-6">
    <UiBaseCard title="Recherches récentes">
      <!-- État vide -->
      <div v-if="searchStore.recentSearches.length === 0" class="text-center py-6">
      ...
      <div v-else class="space-y-3">
        <div class="flex items-center justify-between mb-2">
          <span class="text-[10px] font-black text-hsa uppercase tracking-widest">Historique</span>
          <button @click="searchStore.clearRecentSearches()"
            class="text-[10px] font-medium text-hsa hover:text-red-400 transition-colors">
            Tout effacer
          </button>
        </div>
        <div class="space-y-1">
