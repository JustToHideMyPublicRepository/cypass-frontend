<template>
  <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
      ...
      <UiAppFrame :type="service.status === 'available' ? (service.id === 'secuscan' ? 'terminal' : 'card') : 'card'"
        ...
        <template #headerActions>
          <div v-if="service.status === 'available'" class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full animate-pulse"
              :class="service.theme === 'blue' ? 'bg-primary' : 'bg-success'"></span>
            <span class="text-[9px] md:text-[10px] font-bold uppercase tracking-wider"
              :class="service.theme === 'blue' ? 'text-primary' : 'text-success'">En ligne</span>
          </div>
          <div v-else class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-slate-500"></span>
            <span class="text-[9px] md:text-[10px] font-bold uppercase tracking-wider text-slate-500">Hors ligne</span>
          </div>
        </template>
        ...
        <div class="p-1 h-full flex flex-col relative overflow-hidden group-hover:bg-ash/50 transition-colors">
          ...
          <div class="flex items-start justify-between mb-6 md:mb-8 relative z-10">
            <div
              class="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center text-white shadow-lg transform group-hover:scale-105 transition-transform duration-300"
              ...
              <component :is="service.icon" class="w-6 h-6 md:w-8 md:h-8" />
            </div>

            <NuxtLink v-if="service.status === 'available'" :to="`/modules/${service.id}`"
              class="w-8 h-8 md:w-10 md:h-10 rounded-full border border-ash flex items-center justify-center text-slate-400 hover:bg-ash hover:text-primary transition-all">
          ...
          <div class="relative z-10 flex-grow">
            ...
            <div class="grid grid-cols-2 gap-2 md:gap-3 mb-4 md:mb-6">
              <div v-for="spec in service.specs" :key="spec"
                class="flex items-center gap-2 text-[10px] md:text-xs font-medium text-hsa bg-ash/50 rounded-lg px-2 py-1.5 md:py-1.5">
                <IconCheck class="w-2.5 h-2.5 md:w-3 md:h-3"
