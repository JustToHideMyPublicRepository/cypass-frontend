  ...
  <div v-if="comments.length" class="space-y-6">
    <div v-for="comment in comments" :key="comment.id">
      <RootVigitechDetailCommentsItem :comment="comment" :is-owner="checkIsOwner(comment)"
        :can-edit="checkCanEdit(comment)" :is-editing="editingCommentId === comment.id" :edit-content="editContent"
        ...
        :show-replies="!!showReplies[comment.id]" :loading-replies="!!loadingReplies[comment.id]" :user="user"
        :is-replying="replyingToId === comment.id" :reply-content="replyContent"
        @update:reply-content="$emit('update:replyContent', $event)" :sending-reply="sendingReply"
        :editing-comment-id="editingCommentId" :edit-comment-content="editContent" :saving-comment="savingComment"
        @edit="$emit('edit', $event)" @delete="(id: string, pId?: string) => $emit('delete', id, pId)"
