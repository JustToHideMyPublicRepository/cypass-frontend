<template>
  <div class="space-y-4">
    <h3 class="text-[10px] uppercase px-2">Rapport Hebdomadaire</h3>
    ...
    <div v-for="stat in stats" :key="stat.label"
      ...
      <div class="relative z-10">
        <div class="flex justify-between items-start mb-2">
          <p class="text-[9px] font-black uppercase tracking-widest opacity-80 group-hover:text-BtW transition-colors">
            {{ stat.label }}</p>
          <component :is="stat.icon" class="w-4 h-4" :class="stat.iconColor" />
        </div>
        <div class="flex items-end gap-3">
          <h4 class="transition-transform group-hover:scale-105 duration-500">{{ stat.value }}</h4>
          <div v-if="stat.trend"
        ...
        <p class="text-[10px] font-bold mt-1">{{ stat.sub }}</p>
      </div>
      <div
