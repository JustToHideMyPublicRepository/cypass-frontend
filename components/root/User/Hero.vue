<template>
  <UiBaseCard class="!rounded-[3rem] overflow-hidden !border-none shadow-2xl relative group">
    ...
    <div class="relative p-6 md:p-16 flex flex-col md:flex-row items-center gap-8 md:gap-16">
      <!-- Section Avatar -->
      <div class="relative shrink-0 flex justify-center w-full md:w-auto">
        <div @click="showImageViewer = true"
          class="relative z-10 w-32 h-32 md:w-48 md:h-48 rounded-[2.5rem] md:rounded-[3rem] p-1.5 bg-gradient-to-tr from-primary to-secondary shadow-2xl cursor-pointer hover:scale-105 transition-all duration-500 overflow-hidden group/avatar">
          <div
            class="w-full h-full rounded-[2.3rem] md:rounded-[2.8rem] overflow-hidden bg-ash border-2 md:border-4 border-WtB shadow-inner relative">
      ...
      <div class="flex-1 text-center md:text-left space-y-6">
        <div class="space-y-4 md:space-y-2">
          <div class="flex flex-col md:flex-row items-center justify-center md:justify-start gap-4">
            <h1 class="text-3xl md:text-5xl font-black text-BtW tracking-tighter leading-tight">
            ...
            <div class="flex flex-wrap items-center justify-center gap-2">
              ...
              <UiBaseButton variant="danger" size="sm" v-if="!isOwnProfile && isLoggedIn" @click="emit('report')"
                class="flex items-center gap-2 px-4 py-2 rounded-2xl bg-danger/10 text-danger hover:bg-danger/20 uppercase tracking-widest text-[9px] md:text-[10px] shadow-sm hover:shadow-md active:scale-95 group">
                <IconFlag class="w-3.5 h-3.5 md:w-4 md:h-4 group-hover:rotate-12 transition-transform" />
                Signaler ce compte
              </UiBaseButton>
            </div>
          </div>
          ...
          <p v-if="user.organization_name"
            class="flex items-center justify-center md:justify-start gap-2 text-primary font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] md:text-xs">
        ...
        <div class="flex flex-wrap items-center justify-center md:justify-start gap-x-8 gap-y-6">
          <!-- Date d'inscription -->
          <div class="group/stat">
            <p
              class="text-[9px] md:text-[10px] font-black text-hsa uppercase tracking-widest mb-1 group-hover/stat:text-primary transition-colors">
              Membre depuis</p>
            <div class="flex items-center gap-2 text-BtW font-bold text-base md:text-lg">
              <IconCalendar class="w-4 h-4 md:w-5 md:h-5 text-primary/40" />
              {{ formattedDate }}
            </div>
          </div>
          <!-- Séparateur visuel -->
          ...
          <div class="group/stat">
            ...
            <div class="flex flex-col gap-2">
              <div class="flex items-center gap-2 text-BtW font-bold text-base md:text-lg">
                <IconShield class="w-4 h-4 md:w-5 md:h-5 text-primary/80" />
                {{ publicIncidentsCount }} Incident{{ publicIncidentsCount > 1 ? 's' : '' }}
              </div>
              <div
                class="flex items-center justify-center md:justify-start gap-2 text-hsa font-bold text-[10px] pl-0.5">
                <IconMessage class="w-3 md:w-3.5 h-3 md:h-3.5 text-primary/60" />
