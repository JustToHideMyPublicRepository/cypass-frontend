    ...
    <div class="card relative overflow-hidden group">
      <div class="absolute top-0 left-0 w-full h-1 bg-ashAct/20">
        <div class="h-full transition-all duration-500 ease-out"
          :class="gameState.mode === 'time-attack' && timer < 10 ? 'bg-danger animate-pulse' : 'bg-gradient-to-r from-primary to-secondary'"
          :style="{ width: gameState.mode === 'time-attack' ? `${(timer / 60) * 100}%` : `${((currentIndex + 1) / totalQuestions) * 100}%` }">
        </div>
      </div>
      ...
      <div class="grid gap-3 mt-8">
        <button v-for="(option, index) in shuffledOptions" :key="index" @click="handleAnswer(index)"
...
const {
  ...
  nextQuestion,
  timer
} = inject<any>('quiz-state')

const totalQuestions = computed(() => sessionQuestions.value.length)
const shuffledOptions = computed(() => gameState.shuffledOptions)
const selectedIndex = computed(() => gameState.selectedIndex)
const showFeedback = computed(() => gameState.showFeedback)
</script>
