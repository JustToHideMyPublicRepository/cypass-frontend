...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response: any = await $fetch(`${baseApi}/user/docsentry/archive`, {
      method: 'POST' as any,
      headers: {
    ...
    console.error('Document Archive Proxy Error:', error)
    throw createError({
      statusCode: error.response?.status || 500,
      message: error.data?.message || 'Erreur lors de l’archivage du document'
    })
  }
})
