<template>
  <div class="w-full">
    <form @submit.prevent="handleLogin" class="bg-white dark:bg-slate-800 shadow-xl rounded-2xl p-8 border border-slate-100 dark:border-slate-700">
      ...
      <div class="space-y-4">
        <div>
          <label for="email" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Email Professionnel</label>
          <input 
            v-model="email"
            id="email" 
            type="email" 
            placeholder="nom@gouv.bj"
            required
            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 focus:ring-2 focus:ring-cypass-blue focus:border-transparent bg-white dark:bg-slate-700 dark:text-white transition-shadow"
          />
        </div>
        ...
        <div>
           <div class="flex justify-between items-center mb-1">
             ...
             <a href="#" class="text-xs text-cypass-blue hover:underline">Oublié ?</a>
           </div>
          <input 
            v-model="password"
            id="password" 
      ...
      <div class="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700 text-center">
         <p class="text-sm text-slate-500 dark:text-slate-400">
           ...
           <a href="#" class="text-cypass-blue font-medium hover:underline">Demander un accès</a>
         </p>
      </div>
