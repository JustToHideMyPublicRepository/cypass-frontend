  ...
  <div class="max-w-6xl mx-auto space-y-8 animate-fade-in">
    ...
    <div class="flex items-center justify-between">
      ...
      <div v-if="workspace" class="flex items-center gap-3">
        <UiBaseButton @click="store.openModal(workspace)" variant="primary" size="sm" class="!rounded-xl shadow-lg shadow-primary/20">
    ...
    <div v-else-if="workspace && !loading" class="space-y-8">
      ...
      <div class="relative group">
        <div class="h-48 md:h-64 rounded-[2.5rem] bg-gradient-to-br from-primary/10 via-secondary/5 to-bgClr border border-ash/30 overflow-hidden relative shadow-2xl shadow-ash/10">
          ...
          <div class="absolute bottom-8 left-10 flex items-end gap-6 w-full pr-20">
            <!-- Large Logo -->
            <div class="w-24 h-24 md:w-32 md:h-32 rounded-[2.5rem] border-4 border-bgClr bg-WtB shadow-2xl overflow-hidden shrink-0 group-hover:scale-105 transition-transform duration-500">
              <img :src="getWorkspaceLogoUrl(workspace.logo_url, workspace.name)" :alt="workspace.name" class="w-full h-full object-cover" />
            </div>
            <!-- Identity -->
            <div class="pb-2 space-y-2 flex-1 min-w-0">
              ...
              <div class="flex flex-wrap items-center gap-3">
                <span class="px-3 py-1 rounded-xl bg-ash/20 backdrop-blur-md text-[10px] font-bold text-hsa border border-ash/30 uppercase tracking-widest">
                  <component :is="WORKSPACE_TYPE_CONFIG[workspace.type || 'personal'].icon" class="w-3.5 h-3.5 inline mr-1.5 text-primary" />
                  {{ getWorkspaceTypeLabel(workspace.type) }}
                </span>
                <span class="px-3 py-1 rounded-xl bg-primary/10 backdrop-blur-md text-[10px] font-bold text-primary border border-primary/20 uppercase tracking-widest">
                  <component :is="WORKSPACE_ROLE_CONFIG[workspace.role || 'reader'].icon" class="w-3.5 h-3.5 inline mr-1.5" />
                  {{ getWorkspaceRoleLabel(workspace.role) }}
                </span>
                <span v-if="workspace.members_count" class="px-3 py-1 rounded-xl bg-ash/20 backdrop-blur-md text-[10px] font-bold text-hsa border border-ash/30 uppercase tracking-widest">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div v-if="workspace.id === store.activeWorkspaceId" class="flex items-center gap-4 p-4 rounded-3xl bg-primary/5 border border-primary/20">
             <div class="w-10 h-10 rounded-2xl bg-primary text-WtB flex items-center justify-center shadow-lg shadow-primary/20">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div class="p-6 rounded-[2rem] bg-WtB border border-ash/30 shadow-sm hover:shadow-xl transition-all group/stat">
              <div class="w-12 h-12 rounded-2xl bg-primary/5 flex items-center justify-center text-primary mb-6 border border-primary/10 group-hover/stat:bg-primary group-hover/stat:text-WtB transition-all">
            ...
            <div v-if="workspace.country" class="p-6 rounded-[2rem] bg-WtB border border-ash/30 shadow-sm hover:shadow-xl transition-all group/stat">
              <div class="w-12 h-12 rounded-2xl bg-secondary/5 flex items-center justify-center text-secondary mb-6 border border-secondary/10 group-hover/stat:bg-secondary group-hover/stat:text-WtB transition-all">
            ...
            <div class="p-6 rounded-[2rem] bg-WtB border border-ash/30 shadow-sm hover:shadow-xl transition-all group/stat">
              <div class="w-12 h-12 rounded-2xl bg-warning/5 flex items-center justify-center text-warning mb-6 border border-warning/10 group-hover/stat:bg-warning group-hover/stat:text-WtB transition-all">
                <IconCalendar class="w-6 h-6" />
              </div>
              <p class="text-[10px] font-black text-hsa uppercase tracking-[0.25em] mb-1">Création</p>
              <h3 class="text-lg font-black text-BtW">{{ new Date(workspace.created_at).toLocaleDateString(undefined, { month: 'long', year: 'numeric' }) }}</h3>
            </div>
          </div>

          <!-- Legal & Detailed Info -->
          <UiBaseCard class="p-10 border-ash/30 relative overflow-hidden group/legal rounded-[2.5rem]">
            <div class="absolute top-0 right-0 w-48 h-48 bg-primary/5 rounded-bl-full translate-x-16 -translate-y-16 group-hover/legal:scale-125 transition-transform duration-700"></div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="space-y-6">
                  <div class="flex flex-col gap-2">
                    ...
                    <div class="group/id flex items-center justify-between bg-ash/5 border border-ash/10 p-3 rounded-2xl hover:bg-ash/10 transition-all">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <span class="text-sm font-bold text-BtW">{{ new Date(workspace.created_at).toLocaleString() }}</span>
                  </div>
                </div>
                ...
                <div v-if="workspace.rccm || workspace.ifu" class="space-y-8">
                  ...
                  <div v-if="workspace.ifu" class="flex flex-col gap-2 p-4 rounded-2xl bg-secondary/5 border border-secondary/10">
        ...
        <div class="space-y-8">
          ...
          <div v-if="workspace.id !== store.activeWorkspaceId" class="p-8 rounded-[2.5rem] bg-WtB border border-ash/30 shadow-xl shadow-ash/5 relative overflow-hidden group/switch">
            <div class="absolute top-0 right-0 w-full h-1 bg-gradient-to-r from-primary to-secondary opacity-20 group-hover:opacity-100 transition-opacity"></div>
            
            <div class="flex items-center justify-between mb-8">
              <div class="w-14 h-14 rounded-2xl bg-ash/10 flex items-center justify-center text-hsa shadow-inner border border-ash/20">
            ...
            <p class="text-sm text-hsa mb-8 leading-relaxed font-medium">
              Ce workspace n'est pas votre espace de travail actuel. Toutes vos certifications et dossiers iront ici si vous l'activez.
            </p>
          ...
          <UiBaseCard class="p-8 border-ash/30 shadow-sm rounded-[2.5rem]">
            <div class="flex items-center justify-between mb-8">
              ...
              <NuxtLink to="#" class="text-[10px] font-black text-primary hover:bg-primary/5 px-2 py-1 rounded-lg transition-colors">VOIR TOUT</NuxtLink>
            </div>
            
            <div class="space-y-4">
              <!-- Mock Members for UI Preview -->
              <div v-for="i in Math.min(workspace.members_count || 1, 3)" :key="i" class="flex items-center gap-3 p-4 rounded-2xl bg-ash/5 border border-ash/10 group/item hover:bg-WtB hover:shadow-md transition-all cursor-default">
                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center text-xs font-black text-primary border border-ash/20 group-hover/item:scale-110 transition-transform">
                  {{ i === 1 ? 'AD' : (i === 2 ? 'JD' : 'MK') }}
                </div>
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-black text-BtW truncate">{{ i === 1 ? 'Administrateur' : 'Membre' }}</p>
                  <p class="text-[10px] text-hsa font-bold uppercase tracking-tighter">{{ workspace.role === 'owner' && i === 1 ? 'Propriétaire' : 'Collaborateur' }}</p>
                </div>
              </div>

              <UiBaseButton variant="ghost" class="w-full !rounded-2xl !py-4 border-2 border-dashed border-ash/30 text-hsa hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-all text-xs font-black uppercase tracking-widest">
    ...
    <!-- Not Found -->
    <UiBaseCard v-else-if="!loading" class="p-16 text-center space-y-6 rounded-[3rem] border-ash/30 shadow-2xl shadow-ash/5 max-w-lg mx-auto mt-20">
      <div class="w-24 h-24 rounded-full bg-danger/5 flex items-center justify-center mx-auto text-danger border border-danger/10">
...
useHead({
  title: computed(() => `${workspace.value?.name || 'Workspace'} — CYPASS`),
})
</script>
