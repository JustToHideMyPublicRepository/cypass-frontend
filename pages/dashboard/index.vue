<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
       <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Vue d'ensemble</h1>
       <div class="text-sm text-slate-500 dark:text-slate-400">Dernière mise à jour: Aujourd'hui à 12:45</div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
       ...
       <BaseCard title="Alertes Récentes" class="h-full">
         <template #header>
            <BaseButton variant="secondary" size="sm">Voir tout</BaseButton>
         </template>
         <div class="space-y-4">
             <div v-for="alert in cypassData.alerts" :key="alert.id" class="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                <div :class="[
                ...
                <div>
                   <h4 class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ alert.title }}</h4>
                   <p class="text-xs text-slate-500 mt-1">{{ alert.description }}</p>
        ...
        <BaseCard title="Derniers Documents" class="h-full">
          <div class="overflow-x-auto">
             <table class="w-full text-sm text-left">
                ...
                <tbody>
                   <tr v-for="doc in cypassData.documents.slice(0, 3)" :key="doc.id" class="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                      <td class="px-4 py-3 font-medium text-slate-900 dark:text-white">{{ doc.name }}</td>
                      <td class="px-4 py-3">
