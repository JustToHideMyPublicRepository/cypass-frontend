...
export const useReportUserStore = defineStore('reportUser', {
  ...
  actions: {
    ...
    async getReport(id: string) {
      try {
        const response: any = await $fetch('/api/user/report-user/get-report', {
          params: { id }
        })
    ...
    async receivedReports() {
      ...
      this.error = null
      try {
        const response: any = await $fetch('/api/user/report-user/received-report')
        if (response.success) {
    ...
    async sentReports() {
      ...
      this.error = null
      try {
        const response: any = await $fetch('/api/user/report-user/list-sent-reports')
        if (response.success) {
